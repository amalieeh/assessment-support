...
const Assessment: NextPage = () => {
  ...
  const setAssessment = (assessment: AssessmentType, newScore: number) => {
    ...
    const index = findIndex(assessments, {assessmentId: assessment.assessmentId});
      ...
      <main className={styles.main}>
        ...
        {maxItemsPerPage<=4 ? 
          <div className={styles.grid4answers}>
            {assessments.slice((currentPage * maxItemsPerPage) - maxItemsPerPage, currentPage * maxItemsPerPage).map(
              (assessment: AssessmentType) => <Textbox key={assessment.assessmentId} assessment={assessment} setAssessment={setAssessment}/>
                )}
            </div>
        :  <div className={styles.grid}>
            {assessments.slice((currentPage * maxItemsPerPage) - maxItemsPerPage, currentPage * maxItemsPerPage).map(
